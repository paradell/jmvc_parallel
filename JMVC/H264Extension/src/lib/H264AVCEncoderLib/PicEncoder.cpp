...
//~JVT-W080
  ...
  m_dSumUPSNR               = 0.0;
  m_bInit                   = true;
  m_bParallel				= m_pcCodingParameter->isParallel();
    ...
    while( true )
    {
		...
		if (   (NAL_UNIT_CODED_SLICE_IDR != m_cFrameSpecification.getNalUnitType() || !this->getAVCFlag())&&
            (m_MultiviewRefPicManager.CountNumMultiviewReferenceStreams() > 0) ) {
				...
				//AQUI PETA QUAN S'INTENTA REFERENCIAR UN FRAME AMB EL D'UNA ALTRA VISTA
				//printf("File d'on llegeix per fer la multiview: %s\n",m_MultiviewRefPicManager._references[0]->_fileReader->m_cFileName);
        ...
        UInt    m_uiNumPics     = ( bFieldCoded ? 2 : 1 );
        ...
        //----- encoding -----
        for( UInt uiPicType = 0; uiPicType < m_uiNumPics; uiPicType++ )
        {
...
PicEncoder::finish( PicBufferList&  rcOutputList,
                    PicBufferList&  rcUnusedList )
{
	//printf("clear output i unused\n");
  RNOK( m_pcRecPicBuffer->clear( rcOutputList, rcUnusedList ) );
 // printf("finish clear output i unused\n");
...
PicEncoder::xInitParameterSets()
{
  ...
  RNOK( xDeleteData() );
  //printf("Fem un xCeateData\n");
  RNOK( xCreateData() );

  //===== init objects =====
  //printf("Fem un initSPS\n");
