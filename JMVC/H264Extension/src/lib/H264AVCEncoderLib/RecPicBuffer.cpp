									 ...
									 MultiviewReferenceDirection refDirection)
{
  ...
  if (NOT_MULTIVIEW == refDirection) {
    ...
    // for multiview references and errors will result.
	 // printf("A RecPicBuffer::initCurrRecPicBufUnit() el valor NOT_MULTIVIEW == refDirection \n");
                           ...
                           const SequenceParameterSet&  rcSPS )
{
  ...
  //printf("Valor de uiMaxFramesInDPB: %d",uiMaxFramesInDPB);
  while( uiMaxFramesInDPB-- )
  {
	...
	//printf("\n\npush_back de m_cFreeRecPicBufUnitList a xCCreateData\n");
  }
...
(RecPicBufUnit* pcRecPicBufUnitToRemove)  {
  ...
  printf("\n\npush_back de m_cFreeRecPicBufUnitList a REmoveMultiviewRef\n");
}
...
RecPicBuffer::xOutput( PicBufferList& rcOutputList,
                       PicBufferList& rcUnusedList )
{
	//printf("\n\nArrenca la funcio xOutput\n\n");
	//printf("Tamany m_cFreeRecPicBufUnitList=%d\n",m_cFreeRecPicBufUnitList.size()); //Comença amb valor 12 i va bauxant fins a arribar a 0. Llavors comença a omplir el rec_X.yuv
  ROTRS( m_cFreeRecPicBufUnitList.size(), Err::m_nOK ); //No passa d'aquí fins a la frame 12
	//printf("\n\nfuncio xOutput després de m_cFreeRecPicBufUnitList\n\n");
  ...
  //===== real output =====
  //printf("\nAbans de decidir si fem OutputList->Pushback()\n");
  //printf("iMinPoc = %d\tiMaxPoc=%d\n\n",iMinPoc,iMaxPoc);
  for( Int iPoc = iMinPoc; iPoc <= iMaxPoc; iPoc++ )
  {
    ...
    for( ; iter != end; iter++ )
    {
      if( (*iter)->getPoc() == iPoc ) //No escrivim el primer frame a rec_x.yuv fins que passa això !
      {
		...
		//printf("\n\nIMPORTANT !!!\nFem un push back del OutputList (el rec??)\n");
		...
		//printf("\n\nIMPORTANT !!!\nEstem a xOutput però no fem push back OutputList (el rec??)\n");
    }
  }
                               ...
                               PicBufferList& rcUnusedList )
{
  ...
  while( m_cUsedRecPicBufUnitList.size() )
  {
    ...
    m_cFreeRecPicBufUnitList.push_back( pcRecPicBufUnit );
	//printf("\n\npush_back de m_cFreeRecPicBufUnitList a xClearOutputAll\n");
  }
  return Err::m_nOK;
}
...
RecPicBuffer::xClearBuffer()
{
  ...
  while( cTempList.size() )
  {
    ...
    m_cFreeRecPicBufUnitList.push_back( pcRecPicBufUnit );
	//printf("\n\npush_back de m_cFreeRecPicBufUnitList a xClearBuffer\n");
