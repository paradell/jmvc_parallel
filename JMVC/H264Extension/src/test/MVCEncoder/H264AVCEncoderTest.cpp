                                 ...
                                 Char** argv )
{
  ...
  UInt uiNumberOfLayers = m_pcEncoderCodingParameter->getMVCmode() ? 1 : m_pcEncoderCodingParameter->getNumberOfViews();
  ...
  {
    UInt  uiPriorityId = 0;
    for( UInt uiLayer = 0; uiLayer < m_pcEncoderCodingParameter->getNumberOfViews(); uiLayer++ )
    {
        UInt uiBitplanes;
...
H264AVCEncoderTest::go()
{
  ...
  UInt                    uiNumLayers             = ( m_pcEncoderCodingParameter->getMVCmode() ? 1 : m_pcEncoderCodingParameter->getNumberOfViews() );
  UInt                    uiFrame;
  UInt                    uiLayer;
  ...
  for( bMoreSets = true; bMoreSets;  )
  {
    ...
    RNOK( m_pcH264AVCEncoder      ->writeParameterSets( &cExtBinDataAccessor, bMoreSets) );
		...
		{		
			printf("getScalableSeiMessage a Moresets\n");
		RNOK( m_pcWriteBitstreamToFile->writePacket       ( &m_cBinDataStartCode ) );
...
//SEI }
  ...
  for( uiFrame = 0; uiFrame < uiMaxFrame; uiFrame++ )
  {
    ...
    RNOK( m_pcH264AVCEncoder->process( cOutExtBinDataAccessorList,
                                       ...
                                       &acPicBufferOutputList[0],
                                       &acPicBufferUnusedList[0] ) );
