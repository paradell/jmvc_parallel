...
class EncoderConfigLineBool : public h264::EncoderConfigLineBase
{
public:
  EncoderConfigLineBool( const Char* pcTag, Bool* pbPar, Bool pbDefault ) : EncoderConfigLineBase( pcTag, 6 ), m_pbPar( pbPar )
  {
    *m_pbPar = pbDefault;
  };
  Void setVar( std::string& pvValue )
  {
    *m_pbPar = (Bool)atoi( pvValue.c_str() );
  };
protected:
  Bool* m_pbPar;
};
...
Void EncoderCodingParameter::printHelpMVC(Int     argc,
                                          Char**  argv)
{
  printf("Usage: %s -vf <encoder.cfg> <Num Views>\n\n", argv[0]);
  printf("\n supported options:\n\n");
                                         ...
                                         std::string    rcBitstreamFile[])
{
  std::string acTags[4];
  UInt        uiParLnCount = 0;
  std::string cFileNameStringTemp, cInputFilename,  cInputFile, cReconFile; //Hardcoded a 8, hauria de ser MaxLayers
  ...
  m_pEncoderLines[uiParLnCount++] = new EncoderConfigLineStr ("OutputFile",              &rcBitstreamFile[0],                                      "test.264");
  ...
  m_pEncoderLines[uiParLnCount++] = new EncoderConfigLineStr ("IpAdress",                &m_cIpAdress,                                      "127.0.0.1");
  m_pEncoderLines[uiParLnCount++] = new EncoderConfigLineUInt("UDPPort",                 &m_cPort,                                          5500 );
  ...
  m_pEncoderLines[uiParLnCount++] = new EncoderConfigLineUInt("NumViews",                &m_uiNumberOfLayers, 0);   
  m_pEncoderLines[uiParLnCount++] = new EncoderConfigLineBool("RTP",                &m_uiRTP, 1 );
  m_pEncoderLines[uiParLnCount++] = new EncoderConfigLineBool("Parallel",                &m_uiParallel,                                         1 );
  m_pEncoderLines[uiParLnCount++] = new EncoderConfigLineBool("Debug",                  &m_uiDebug,                                            1 );
  m_pEncoderLines[uiParLnCount++] = new EncoderConfigLineBool("Verbose",                  &m_uiVerbose,                                            0 );
...
//  CodingParameter::SpsMVC.setViewCodingOrder(m_cViewOrder);
  ...
  /////////////////////////
	...
	////
      ...
      if( m_uiMVCmode )
      {
		  // AQUI HEM DE CREAR NOVES ENTRADES PER INPUT & OUTPUT
		  //Creariem dues vegades el mateix arxiu ?? ARA s'intenta crear dues vegades el arxiu de lectura piri_2.yuv
		  //printf("\nNumber of Layers at EncoderCodingParameter::xReadFromFile: %d\n",m_uiNumberOfLayers);

		  //Creem un string per guardar l'arrel del fitxer de sortida
			rcBitstreamFile[1]=rcBitstreamFile[0];		

			for(UInt i=0;i<m_uiNumberOfLayers;i++){
		
				//m_uiNumberOfLayers = 0;
				xAppendStringWithNO ( cInputFile, cFileNameStringTemp, /*m_CurrentViewId*/i, ".yuv");
				getLayerParameters(i).setInputFilename  ( (Char*)cFileNameStringTemp.c_str() );
				
				if(m_uiVerbose)
					printf("Llegirem de l'arxiu: %s\n",(Char*)cFileNameStringTemp.c_str());
				
				if(!m_uiParallel){
					xAppendStringWithNO ( cReconFile, cFileNameStringTemp, /*m_CurrentViewId*/i, ".yuv"); //crea només un REC file
					getLayerParameters(i).setOutputFilename ( (Char*)cFileNameStringTemp.c_str() );
					
					if(m_uiVerbose)
						printf("Hem creat l'arxiu: %s\n",(Char*)cFileNameStringTemp.c_str());
				}

				getLayerParameters(i).setFrameWidth     ( m_uiFrameWidth );
				getLayerParameters(i).setFrameHeight    ( m_uiFrameHeight );
				xAppendStringWithNO ( rcBitstreamFile[i], rcBitstreamFile[i], /*m_CurrentViewId*/i, ".264");

				/*//Per la vista 1
				xAppendStringWithNO ( cInputFile, cFileNameStringTemp, m_CurrentViewId+1, ".yuv");
				getLayerParameters(1).setInputFilename  ( (Char*)cFileNameStringTemp.c_str() );
				printf("Llegirem de l'arxiu: %s\n",(Char*)cFileNameStringTemp.c_str());
				xAppendStringWithNO ( cReconFile, cFileNameStringTemp, m_CurrentViewId+1, ".yuv");
				getLayerParameters(1).setOutputFilename ( (Char*)cFileNameStringTemp.c_str() );
				printf("Hem creat l'arxiu: %s\n",(Char*)cFileNameStringTemp.c_str());
				//printf("Hem creat l'arxiu: %s\n",getLayerParameters(1).getOutputFilename);

				getLayerParameters(1).setFrameWidth     ( m_uiFrameWidth );
				getLayerParameters(1).setFrameHeight    ( m_uiFrameHeight );
				xAppendStringWithNO ( rcBitstreamFile[1], rcBitstreamFile[1], m_CurrentViewId+1, ".264"); //Es creen dues
				//printf("Escriurem a l'arxiu: %s",rcBitstreamFile[1]);
				//printf("\n\nHauriem de tenir dues sortides:\n%s\n\n",rcBitstreamFile[0]); //Error al imprimir els noms dels arxius*/
			}
      }
